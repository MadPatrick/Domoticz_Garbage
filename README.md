...
| **Garbage Container** | Shows a "Today / Tomorrow" reminder (see [Notification device](#notification-device)) |
...
| **Language / Taal** | Output language for labels and date names: `NL` = Nederlands, `EN` = English | `NL` |
...
| `TodayTime` | Until what time the "Today" label is shown (HH:MM) | `16:00` |
...
| `LabelToday` | Label used when the pickup is today | `Today` (EN) / `Vandaag` (NL) |
| `LabelTomorrow` | Label used when the pickup is tomorrow | `Tomorrow` (EN) / `Morgen` (NL) |
| `LabelNoData` | Text shown when no upcoming pickup data is available | `No collection data available` (EN) / `Geen verzamelgegevens beschikbaar` (NL) |

> **Note:** If `LabelToday`, `LabelTomorrow`, or `LabelNoData` are set in `config.txt`, those values always take precedence over the **Language / Taal** selector. Remove or comment out those entries in `config.txt` to let the language selector control the labels automatically.
...
Pickup dates are always displayed in the format **`wd dd mmm`** (e.g. `ma 07 apr` in NL, `Mon 07 Apr` in EN).
...
|---|---|
| Collection day, before `TodayTime` | `<LabelToday> <type>` (e.g. `Today Restafval`) |
| Day before collection, from `TomorrowTime` | `<LabelTomorrow> <type>` (e.g. `Tomorrow PMD`) |
| Outside these windows | *(empty)* |

The reminder times and labels can be adjusted in `config.txt` (see [config.txt settings](#configtxt-settings)).  
...
|---|---|
| Collection day, at or after `NotifyTime` | `<LabelToday>: <type>` (e.g. `Today: Restafval`) |
| Day before collection, at or after `NotifyTime` | `<LabelTomorrow>: <type>` (e.g. `Tomorrow: PMD`) |
...
| "Garbage Container" device not updating | Check `TodayTime` and `TomorrowTime` in `config.txt`; restart the plugin after saving. |
| No e-mail / push notification received | Enable notifications via the **Enable notification** dropdown in the hardware panel and verify `NotifyTime` in `config.txt`. Also check that notification channels are configured under **Setup → Settings → Notifications** in Domoticz. |
